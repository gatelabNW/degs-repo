...
dirs$fsmresfiles_project_root <- Sys.getenv('dirs.fsmresfiles_project_root') |> mkdirs()
dirs$objects <- Sys.getenv('dirs.objects') |> mkdirs()
dirs$seurat <- Sys.getenv('dirs.seurat') |> mkdirs()
...
"{dirs$scripts}/lib/general/toolbox.R" |> glue() |> source()
